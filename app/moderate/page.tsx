...
export default async function ModeratePage() {
  ...
  const [{ data: papers }, { data: materials }] = await Promise.all([
    supabase
      .from('past_papers')
      .select('id, year, exam_type, file_url, created_at, uploaded_by, courses(code, name), profiles(id, student_id, full_name)')
      .eq('status', 'pending')
      .order('created_at', { ascending: true }),

    supabase
      .from('study_materials')
      .select('id, title, content_type, week_number, file_url, created_at, uploaded_by, courses(code, name), profiles(id, student_id, full_name)')
      .eq('status', 'pending')
      .order('created_at', { ascending: true }),
  ]);
