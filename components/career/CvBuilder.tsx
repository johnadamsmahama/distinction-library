          ...
          <>
            ...
            <Section num={4} label="Education">
              <div className="space-y-3">
                {education.map((row, i) => (
                  <div key={i} className="bg-white border border-g100 rounded-none p-3 space-y-2">
                    <input
...
function ModeBtn({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`flex-1 font-condensed font-bold text-xs uppercase tracking-wide px-4 py-3 rounded-none transition-all ${
        active
