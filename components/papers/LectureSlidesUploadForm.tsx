...
const placeholderText = 'font-display italic text-[15px] text-[#8A8471]';
...
// picker page. Used for the form as well as the suspended/submitted states,
// so every state of this page shares the same identity.
...
export default function LectureSlidesUploadForm({
  courses,
  uploadSuspended,
}: {
  courses: CourseOption[];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!week || Number(week) < 1 || Number(week) > 12) {
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className={fieldRow}>
            ...
            <input
              ...
              className={`w-full bg-transparent outline-none ${fieldUnderline} ${title ? filledText : ''} placeholder:italic placeholder:font-display placeholder:text-[15px] placeholder:text-[#8A8471] ${title ? '' : 'font-display'}`}
          ...
          <div className={fieldRow}>
            ...
            <input
              ...
              className={`w-full bg-transparent outline-none ${fieldUnderline} ${week ? filledText : ''} placeholder:italic placeholder:font-display placeholder:text-[15px] placeholder:text-[#8A8471] ${week ? '' : 'font-display'}`}
          ...
          <button
            type="submit"
            disabled={loading}
            className={`w-full mt-4 border-2 border-[#B22222] text-[#B22222] bg-transparent ${mono} font-bold text-[12px] uppercase tracking-wide py-3.5 disabled:opacity-60 flex items-center justify-center gap-2 hover:bg-[#B22222] hover:text-[#F6F1E3] transition-colors`}
          >
...
function statusColor(status: string): string {
  ...
  if (status === 'skipped_duplicate') return 'text-[#8A8471]';
...
function BulkUploadPanel() {
  ...
  const handleZipUpload = async () => {
    ...
    const { data: job, error: insertErr } = await supabase
      .from('bulk_upload_jobs')
      .insert({ uploaded_by: user.id, zip_path: path, job_type: 'material' })
      .select('id')
      .single();
    setUploading(false);
      ...
      <div>
        <div className="mb-2.5">
          <div className="flex items-center justify-between mb-1.5">
            ...
            <span className={`${mono} text-[9.5px] text-[#8A8471]`}>
