...
export default async function Stats() {
  ...
  return (
    <section className="relative bg-navy py-20 px-7 overflow-hidden">
      <div
      ...
      />
      <div className="relative z-10 max-w-content mx-auto flex flex-wrap justify-center gap-[28px]">
        {STATS.map((s, i) => (
          <div
            key={s.label}
            className="relative w-[148px] h-[153px] border border-gold/50 flex flex-col items-center justify-center text-center pt-1 before:content-[''] before:absolute before:inset-[8px] before:border before:border-gold/30"
            style={{ transform: `rotate(${[-3, 2, -2, 4][i % 4]}deg)`, marginTop: [0, 10, 0, 8][i % 4] }}
          >
            <svg
              viewBox="0 0 24 24"
              className="relative z-10 w-[23px] h-[23px] mb-2 stroke-gold fill-none"
              strokeWidth={2.2}
            >
              <path d={s.path} />
            </svg>
            <div className="relative z-10 font-display font-black text-[26px] text-gold-light">{s.num}</div>
            <div className="relative z-10 font-condensed font-extrabold text-[9.5px] uppercase tracking-[.07em] text-white/70 mt-1.5">
              {s.label}
            </div>
          </div>
        ))}
